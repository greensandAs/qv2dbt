...
def write_xlsx(script: QvScript, lin: Lineage, path: str) -> None:
    try:
        ...
        # openpyxl not available — write a CSV fallback instead
        import csv
        with open(path.replace(".xlsx", ".csv"), "w", newline="", encoding="utf-8") as f:
            w = csv.writer(f)
            w.writerow(_HEADERS)
            for t in script.tables:
                for c in lin.for_table(t.name):
                    w.writerow([
                        t.layer, t.name, c.column, c.mapping_type,
                        ", ".join(f"{a}.{b}" for a, b in c.ultimate_sources),
                        "", c.qlik_expr, c.snowflake_sql, "; ".join(c.notes),
                    ])
        return
